...
/// <summary>Converts a decoded <see cref="Image"/> to the pixel format requested via <see cref="DecoderOptions.TargetPixelFormat"/>. The actual pixel reshaping is done by the shared, SIMD-optimized <see cref="PixelFormatConversionKernels"/> — this class only owns the supported-pair dispatch (scoped to the formats <see cref="GifDecoder"/> actually produces: Rgb24/Rgba32) and GIF's own exception type.</summary>
internal static class PixelFormatConverter
{
    ...
    public static Image ConvertIfNeeded(Image image, PixelFormat? target)
    {
        ...
        var converted = (image.PixelFormat, targetFormat) switch
        {
            ...
            _ => throw new GifDecodingException($"Cannot convert decoded {image.PixelFormat} pixels to requested format {targetFormat}."),
        };
