...
/// <summary>Converts a decoded <see cref="Image"/> to the pixel format requested via <see cref="DecoderOptions.TargetPixelFormat"/>. Avif's own copy — mirrors Bmp/Gif/Jpeg/Webp's converters, scoped to the formats <see cref="AvifDecoder"/> actually produces (Rgb24/Gray8 for now).</summary>
internal static class PixelFormatConverter
{
    ...
    public static Image ConvertIfNeeded(Image image, PixelFormat? target)
    {
        ...
        var converted = (image.PixelFormat, targetFormat) switch
        {
            ...
            (PixelFormat.Gray8, PixelFormat.Rgb24) => Expand(image, 1, 3, static (src, dst) => { dst[0] = src[0]; dst[1] = src[0]; dst[2] = src[0]; }),
            (PixelFormat.Gray8, PixelFormat.Rgba32) => Expand(image, 1, 4, static (src, dst) => { dst[0] = src[0]; dst[1] = src[0]; dst[2] = src[0]; dst[3] = 255; }),
