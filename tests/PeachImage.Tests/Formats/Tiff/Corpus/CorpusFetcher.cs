...
/// <summary>
/// Downloads the <c>tiff-conformance</c> subtree of the Imazen <c>codec-corpus</c> repository into the
/// gitignored <see cref="CorpusPaths.Root"/>, using the GitHub Git Trees API to enumerate blobs under that
/// subpath and <c>raw.githubusercontent.com</c> to fetch each one — no <c>git</c> executable required, and
/// no full-repo clone. Mirrors the Bmp corpus fetcher's approach, scoped to a single repo/path.
/// </summary>
internal static class CorpusFetcher
{
    private static readonly (string Owner, string Repo, string Branch, string[] Paths, string Destination) ImazenSource =
        ("imazen", "codec-corpus", "main", ["tiff-conformance"], "imazen-codec-corpus");
